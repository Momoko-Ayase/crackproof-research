---
description: Crackproof 受保护 PE 文件的磁盘容器布局、加密头部及其密钥派生，以及受保护文件如何被识别、分类与拆分为 stub/伴生体文件对。
---
...
因此 payload 的搬运为：文件区间 `[info[4] + 4096, info[4] + 4096 + info[5])` → 映像区间 `[info[3], info[3] + info[5])`。前 `decrypt_size = info[6] - info[3] + 8192` 字节以滚动 XOR 链解密（见[数据变换原语](primitives.md)）；其余原样拷贝。随后文件前 4096 字节被覆盖回映像作为其头部。
...
- **TLS 目录**——`IMAGE_TLS_DIRECTORY` 结构体、其原始数据模板与数据目录项，这些都被加壳器从加密 payload 中剥离（见 [PE 变换](pe-transformations.md)）。
- 真实的 `DllCharacteristics` 字段与基址重定位表——伴生模块**不是** `/FIXED`，与较旧的单文件构建不同。

关于此文件对在启动时如何加载，见[运行时行为](runtime-behavior.md)；关于缺失部分如何被还原进重建映像，见 [PE 变换](pe-transformations.md)。
