...
受保护二进制的首个执行线程属于 Crackproof，而非程序。加载器沿固定管线行进：环境检查、反分析清扫、内核驱动与子模块设置，然后是分阶段加载器中的分阶段解密，再然后——可选地——把它刚解密的内容重新加密，最后才跳转到原始入口点（OEP）。本页描述在运行时观察到的这条管线。
...
页错误处理器不住在受保护模块的映像内。它随一个手动映射的支持模块（`HtdpStub2.dll`——见内核驱动与子模块）分发，这就是为什么只转储主模块时找不到处理器签名。
...
同一阶段内含自己的 `.reloc` 遍历器：重定位由加载器施加（状态 `655`），而非 OS——与 PE 变换中的 `/FIXED` 处理一致。
