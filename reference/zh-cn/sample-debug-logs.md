---
description: "从受保护进程捕获的真实 CrackProof 调试日志——页加密的宿主 EXE、仅整体解密的原生 DLL 与托管 DLL——已脱敏并附注解。"
---

# 调试日志示例

这些是真实的 CrackProof 调试日志，捕获方式是在 `%temp%` 下创建该可执行文件对应的 12 位十六进制字符文件夹并启动受保护程序（见[运行时行为](https://app.gitbook.com/s/fEb9nKPvKsjkPAHMUbOt/runtime-behavior#tiao-shi-ri-zhi)）。路径与产品名已替换为通用占位符；其余一切——选项标志、状态码、地址、hook 列表——均为原文。
...
* **第 3/4 行**——时间戳与模块的加载基址。
* **后续行**——12 位[状态码](https://app.gitbook.com/s/fEb9nKPvKsjkPAHMUbOt/runtime-behavior#qi-dong-xu-lie-yu-zhuang-tai-ma)，每个 stage 一行；缩进的 `000`–`00N` 行携带 stage 特定的细节（地址、计数、被 hook 的函数）。
...
* `C03` 给出驱动代际：`Htsysm7679`——第二代 Htsysm（见[内核驱动与子模块](https://app.gitbook.com/s/fEb9nKPvKsjkPAHMUbOt/kernel-components)）。
