...
* `C03` 给出驱动代际：`Htsysm7679`——第二代 Htsysm（见 [Htsysm 内核组件](https://app.gitbook.com/s/fEb9nKPvKsjkPAHMUbOt/runtime/kernel-components)）。
...
* `A15` 跟在宿主进程检查之后。它是不带检查体的纯标记：宿主验证已通过，启动进入解密前奏。
