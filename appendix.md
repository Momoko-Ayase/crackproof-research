...
- Status codes and the boot sequence: [Runtime behavior](runtime-behavior.md#the-boot-sequence-and-status-codes)
- Module codes and driver generations: [Kernel drivers and submodules](kernel-components.md)
- il2cpp metadata constants: [il2cpp metadata obfuscation](il2cpp-metadata.md)
- Huffman table/token format: [Data transformation primitives](primitives.md#the-huffmanlz-compression-format)

## Verification suite

Every Python snippet in this document was verified before publication: each function's output was compared byte-for-byte against an independent reference port of the algorithms on identical inputs (30 test vectors, all matching). The complete, runnable snippet files and the comparison harness are in the [`verification/`](https://github.com/Momoko-Ayase/crackproof-research/tree/main/verification) directory of the source repository — `python run_tests.py` re-runs the full comparison.
