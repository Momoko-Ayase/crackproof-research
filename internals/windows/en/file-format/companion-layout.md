...
- The **TLS directory** — the `IMAGE_TLS_DIRECTORY` struct, its raw-data template, and the data-directory entry, all of which the packer strips from the encrypted payload (see [PE transformations](../pe-reconstruction/README.md)).
...
For the boot-time view of how this pair is loaded, see [Runtime behavior](../runtime/README.md); for how the missing pieces are restored into a reconstructed image, see [PE transformations](../pe-reconstruction/README.md).
