...
**Fourth/Fifth/Seventh.** Unwrapped from quads at `dp_base + 0x40/0x50/0x70` with keys of the form `header_checksum ^ crc32(previous region) ^ advance_key(seed)`, where each seed comes from the stage decrypted just before. The `advance_key` round count is not fixed: it is the number of leading active helpers in the stage's four-slot table of 16-byte descriptors at `dp_base`, where a slot is active while its size dword at `+4` is greater than 4 and the first empty slot ends the count even if later slots are nonempty. The Seventh stage contains the first **bytecode stub** (located by backward-then-forward LFSR scan).

**EighthStage and its key search.** The Eighth stage key is not stored either: candidates are collected from gap heuristics around the stub (end-of-region gaps `0xD0…0x100`, stub-relative gaps, and a linear scan for non-printable dwords), each run through `advance_key` with the round count taken from the same active-helper rule on the table at `dp_base + 0x80`, and each tried against the full stage-decrypt composite. The winner is the candidate whose result **decompresses cleanly and points inside the image** — a concrete success condition supplied by the Huffman decoder.
