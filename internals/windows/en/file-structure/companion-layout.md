...
A descriptor's `src` field therefore maps to file offset `src + section_data_file_base`. The same formula appears in every build family (it's referenced as both `rebase` and `compress_data_offset` in loader code).
