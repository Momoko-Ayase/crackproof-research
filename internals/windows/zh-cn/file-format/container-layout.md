...
因此 payload 的搬运为：文件区间 `[info[4] + 4096, info[4] + 4096 + info[5])` → 映像区间 `[info[3], info[3] + info[5])`。前 `decrypt_size = info[6] - info[3] + 8192` 字节以滚动 XOR 链解密（见[数据变换原语](../data-transforms/README.md)）；其余原样拷贝。随后文件前 4096 字节被覆盖回映像作为其头部。
