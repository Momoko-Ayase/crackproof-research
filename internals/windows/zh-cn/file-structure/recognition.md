...
实际样本中的受保护文件在四个彼此独立的维度上各不相同，所有组合都需要处理。

**位宽。** PE32+（可选头 magic `0x20B`，64 位）与 PE32（`0x10B`，32 位）容器共享头部/payload 层，但使用完全不同的配置布局、阶段结构与最终 PE 修补。

**对象种类。** 原生 EXE、托管 EXE、原生 DLL 与托管 DLL（见[识别与分类](#识别与分类)）。某些托管布局会在受保护文件中原样保留 COR20 头与 BSJB 元数据流。这不表示托管方法体是明文；方法体仍属于需要按正常流程恢复的节。
...
- **EXE 风格外壳**（较新）：DLL 被套上与 EXE 相同的外壳布局。[外置伴生体拆分](companion-layout.md)即属于这一家族。
