...
受保护文件仍是 PE 文件，但原始映像已被加密容器替换。容器在文件前部保留原始 PE 头（基本可读），在固定偏移处以密钥派生函数隐藏一个小参数块，其余一切（程序的节与加载器自身的代码）都以加密且通常压缩的 payload 形式存储。
...
| `info[1]` | **格式 magic**，标识保护构建（见[格式 magic](#格式-magic)） |
| `info[2]` | 保留/变体                                      |
