...
* 诸如 `info[3]` 的名称指从加密文件头派生的 8-dword 表中的条目（见[受保护文件格式](file-format/README.md)）。

## 内容索引

| 范围 | 起点 |
|---|---|
| 磁盘布局与构建识别 | [受保护文件结构](file-format/README.md) |
| 密码、压缩与校验和 | [数据变换](data-transforms/README.md) |
| 加载器阶段与节恢复 | [加载与节恢复](loading/README.md) |
| 头、目录、导入与 CLR 数据 | [PE 重建](pe-reconstruction/README.md) |
| 启动检查、页保护与内核组件 | [运行时行为](runtime/README.md) |
| 分析流程、局限与常量 | [分析笔记](analysis/README.md) |

Windows 数据变换页面对应的测试代码发布在[验证与参考](https://app.gitbook.com/s/L9bXLua8yrIPEUZOHO21/readme)中。
