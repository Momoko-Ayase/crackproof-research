...
驱动名中编码了构建戳：`Htsysm7679` 与 `Htsysm767901`（即 `7679` 变体 `01`）。同一 dword `0x00007679` 也出现在受保护文件内部，作为 32 位加载器最终阶段的**配置簇戳**（见[分阶段加载器](../loading-and-pe-repair/loading/)），以及 stage-5 标记表内一个 8 字节标签的一半。它充当把受保护构建与其驱动世代联系起来的版本标识符，在分类未知样本时是有用的指纹。
