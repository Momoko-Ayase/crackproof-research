...
由[滾動 XOR 鏈](../../data-transforms/rolling-and-rotation.md#滾動密鑰家族)解出的 payload 主體並不是程序。它是加載器的配置表、加密的階段代碼，以及最終用來恢復程序節的描述符。加載器是**自解密的**：其代碼被拆分為多個階段，每個階段都用在上一階段正確解密後才存在的內容派生的密鑰加密。無論是運行時還是靜態分析，都必須按序穿過各階段；沒有直達最終階段表的捷徑。
...
配置塊的位置隨構建浮動，因此靠掃描定位：在 `[info[6] + 1000, info[6] + 8000)` 內找**錨點**——一個等於 `info[3]` 的 dword，其 `+8` 處的值略小於 `info[6]`（差值 ≤ `0x1000` 且為 `0x200` 的倍數）。自偏移 40 起的字段再按 `anchor_extra ∈ {0, 8}` 平移。

可靠的判別是 `anchor + 120 + extra` 處的 stage-1 描述符本身：其 `(base, length)` 須滿足 `base ≥ info[3]`、`length ≥ 16`，且 `base + length` 落在映像內。先試 `extra = 0` 再試 `extra = 8`，取第一對通過校驗的。附近常有一個版本風格的 dword（多在 `anchor + 104` 或 `anchor + 112`，高半字節為 `0x4`），在舊家族裡常見，但並非所有構建都穩定，只能當輔助信號。
...
最後階段重建 PE 頭：節錶轉換、加密的入口點/數據目錄塊、TLS 處理、`/FIXED` 策略、代碼頁擾動與託管元數據處理。這些是格式變換而非加載器階段，見 [PE 重建](../pe-reconstruction/README.md)。
