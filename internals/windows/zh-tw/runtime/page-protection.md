...
頁錯誤處理器不在受保護模塊的映像內。它隨一個手動映射的支持模塊 `HtdpStub2.dll`（見[手動映射輔助模塊](mapped-modules.md)）分發，因此只轉儲主模塊時找不到處理器簽名。
...
同一階段內含自己的 `.reloc` 遍歷器：重定位由加載器施加（狀態 `655`），而非 OS——與 [PE 重建](../loading-and-pe-repair/pe-reconstruction/README.md)中的 `/FIXED` 處理一致。
