...
實際樣本中的受保護文件在四個彼此獨立的維度上各不相同，所有組合都需要處理。

**位寬。** PE32+（可選頭 magic `0x20B`，64 位）與 PE32（`0x10B`，32 位）容器共享頭部/payload 層，但使用完全不同的配置佈局、階段結構與最終 PE 修補。

**對象種類。** 原生 EXE、託管 EXE、原生 DLL 與託管 DLL（見[識別與分類](#識別與分類)）。某些託管佈局會在受保護文件中原樣保留 COR20 頭與 BSJB 元數據流。這不表示託管方法體是明文；方法體仍屬於需要按正常流程恢復的節。
...
- **EXE 風格外殼**（較新）：DLL 被套上與 EXE 相同的外殼佈局。[外置伴生體拆分](companion-layout.md)即屬於這一家族。
