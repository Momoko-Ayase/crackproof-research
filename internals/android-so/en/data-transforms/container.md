...
Each segment is a word cipher, then an optional AES-256-CBC pass (zero IV) unless the header requested a skip, then a writer. The word cipher uses a rolling `w0/w6/w7` state with a fixed constant set (`0x8e495673`, `0xe9fa57e4`, `0x72f6fcbe`, `0x4f8b1bca`, `0xb43b9baf`, `0xaf57f7fb`, `0x07b48238`, `0xe34eac63`). The scratch value `w7 ^ w6` overwrites `w0` and carries into the next iteration.
...
The writer stream starts with a 16-byte header `{base, count, table_offset, src_base}`, then `count` entries `{dst, out_len, src_len, pad}`. A write is a raw copy when `out_len == src_len`, otherwise a [compressed block](compression.md). For either form, the decoder requires:
