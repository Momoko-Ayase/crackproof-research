...
The format is described from the outer header first, then the record stream inside it. Bounds checks are part of the format description because a plausible field isn't enough to identify a valid file. Load-time discovery of the same header is in [Stage 1 bootstrap](../runtime/stage1-bootstrap.md).
