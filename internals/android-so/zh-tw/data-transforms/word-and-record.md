---
description: "GF(2^32) 詞混合、流首部與記錄密碼，以及 0x9D 作業記錄頭。"
---

# 詞、流與記錄密碼

Android 記錄流不復用 Windows 的滾動 XOR 家族。詞混合是 GF(2³²) 乘法。流首部、記錄描述符和 `0x9D` 作業記錄頭各自在這一乘法外包一層不同的反饋規則。常數隨家族而變；下面的值是已觀察材料，不是唯一通用密鑰。
...
32 字節參數頭與第二階段映像使用無符號 32 位迴繞。對詞下標 `i` 與家族常數 `C`：
...
已觀察到兩套常數。它們共享 `sid * 0x9D323CD7` 乘積和對首部詞的 `gf` 混合；附加常數與移位組裝不同。
...
`G` 是該整條流的描述符密碼種子。無論構建使用哪套常數，[第二階段記錄流](../file-format/stage2-streams.md)中的結構檢查都仍然有效。
...
`(off + 3) & 5` 是按位與，不是常數 3：第 0 詞移 1、第 1 詞移 5，隨後交替。把它讀成常數 3 會靜默解出垃圾。`G` 對整條流恆定；`idx` 改變每條描述符的 `record_mask`；`feedback` 攜帶前一個*密文*詞。
