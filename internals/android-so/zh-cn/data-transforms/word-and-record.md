---
description: "GF(2^32) 词混合、流首部与记录密码，以及 0x9D 作业记录头。"
---

# 词、流与记录密码

Android 记录流不复用 Windows 的滚动 XOR 家族。词混合是 GF(2³²) 乘法。流首部、记录描述符和 `0x9D` 作业记录头各自在这一乘法外包一层不同的反馈规则。常数随家族而变；下面的值是已观察材料，不是唯一通用密钥。
...
32 字节参数头与第二阶段映像使用无符号 32 位回绕。对词下标 `i` 与家族常数 `C`：
...
已观察到两套常数。它们共享 `sid * 0x9D323CD7` 乘积和对首部词的 `gf` 混合；附加常数与移位组装不同。
...
`G` 是该整条流的描述符密码种子。无论构建使用哪套常数，[第二阶段记录流](../file-format/stage2-streams.md)中的结构检查都仍然有效。
...
`(off + 3) & 5` 是按位与，不是常数 3：第 0 词移 1、第 1 词移 5，随后交替。把它读成常数 3 会静默解出垃圾。`G` 对整条流恒定；`idx` 改变每条描述符的 `record_mask`；`feedback` 携带前一个*密文*词。
