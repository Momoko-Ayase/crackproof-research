...
- **Bilingual parity.** Every content change goes into both language variants of the affected Space. Keep the published GitBook page paths and `SUMMARY.md` hierarchy identical; GitBook may normalize the synced source directory or overview filename differently after a group slug is renamed. Code blocks stay byte-identical across languages.
...
- **Navigation parity.** Keep the English and Chinese `SUMMARY.md` trees structurally identical. Use `##` headings for major page groups and indent only genuine parent-page children below their parent (for example, Loading and PE repair → Loading and section recovery → Stage chain). Keep generic Overview pages as siblings when a group has no genuine parent page. GitBook records a manually chosen group slug in the changed `SUMMARY.md` heading as an HTML anchor, for example `## 文件结构 <a href="#file-structure" id="file-structure"></a>`. The English and Chinese variants must resolve to the same ASCII group slug; mirror the anchor when both source files are edited, and do not transliterate Chinese titles. Pinyin paths must never be used as group slugs or canonical Chinese page paths.
