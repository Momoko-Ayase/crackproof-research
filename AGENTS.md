...
- **Navigation parity.** Keep the English and Chinese `SUMMARY.md` trees structurally identical. Use `##` headings for major page groups and indent only genuine parent-page children below their parent (for example, Loading and PE repair → Loading and section recovery → Stage chain). Keep generic Overview pages as siblings when a group has no genuine parent page. Git Sync does not expose page-group slugs in `SUMMARY.md`; after sync, set each English/Chinese group slug to the same ASCII value in GitBook's group Rename dialog. Pinyin paths must never be used as group slugs or canonical Chinese page paths.
