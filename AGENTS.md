...
- **Publishing.** EN and zh are Git-Synced: pushing to `main` publishes both (the EN space is edit-locked). The `reference/` section is published through the GitBook MCP change-request flow until its Git Sync is wired.
