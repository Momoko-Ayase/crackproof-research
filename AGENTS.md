...
These rules apply to all content in this repository — the English pages in `internals/en/`, the Simplified Chinese mirror in `internals/zh-cn/`, and the bilingual Verification & reference section in `reference/`.
...
- **Bilingual parity.** Every content change goes into both languages (`internals/en/` + `internals/zh-cn/`, `reference/en/` + `reference/zh-cn/`), with identical file names and structure. Code blocks stay byte-identical across languages.
...
- **Publishing.** All four spaces are Git-Synced — `internals/en/` (English, edit-locked), `internals/zh-cn/` (简体中文), `reference/en/`, `reference/zh-cn/` — so pushing to `main` publishes everything. The reference pages use GitBook-side file names (`detect.py.md`, `sample-debug-logs.md`, …); each language's `SUMMARY.md` is the page list. Cross-space links use GitBook's native internal-link form `https://app.gitbook.com/s/<spaceId>/<path>` (never the public hostname), always targeting the matching language: internals EN `PuKTEy2soDgSB3qfWACy`, internals zh `fEb9nKPvKsjkPAHMUbOt`, reference EN `8S0xnfw9UP9A2yylicaA`, reference zh `L9bXLua8yrIPEUZOHO21`.
