...
- **Publishing.** All four spaces are Git-Synced — `internals/en/` (English, edit-locked), `internals/zh-cn/` (简体中文), `reference/en/`, `reference/zh-cn/` — so pushing to `main` publishes everything. The reference pages use GitBook-side file names (`detect.py.md`, `sample-debug-logs.md`, …); each language's `SUMMARY.md` is the page list.
