...
- **Navigation parity.** Keep the English and Chinese `SUMMARY.md` trees structurally identical. Each major section uses a real parent/index page whose ASCII filename is the same in both languages (for example, `file-structure.md` and `data-transforms.md`); its localized title may differ. Put the Overview page and topic pages below that parent. GitBook then gives both languages the same canonical URL path. Pinyin paths are legacy aliases only and must never be the canonical path of a Chinese page.
